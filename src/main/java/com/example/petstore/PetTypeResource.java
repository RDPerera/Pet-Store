...
public class PetTypeResource {
    ...
    List<String> petTypes = new ArrayList<String>(Arrays.asList("Dog","Cat","Bird","Rabbit","Fish"));
    ...
    //Create new Pet type
    @APIResponses(value = {
            @APIResponse(responseCode = "200", description = "Create a pet type", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(ref = "Pet")))})
    @POST
    @Path("create")
    public Response createPetType(@FormParam("type") String pet_type) {
    ...
    @PUT
    @Path("update")
    public Response updatePetType(@FormParam("pet_type") String petType,
                                  @FormParam("pet_type_id") int petType_id
    ) {
